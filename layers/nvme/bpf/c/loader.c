...
static void write_counters(struct standalone_bpf *skel, const char *csv_path) {
  ...
  /* Untracked completions (per disk+op): completions whose command was never
   * tracked at setup — excluded by the device/proc/container filter, or a
   * setup-probe miss. Build a JSON object from the hash map. */
    ...
    while (bpf_map_get_next_key(ufd, prev, &next_key) == 0) {
      ...
      if (bpf_map_lookup_elem(ufd, &next_key, &uval) == 0 && uval > 0) {
        ...
        if (uoff < sizeof(untracked_json))
          uoff += snprintf(untracked_json + uoff, sizeof(untracked_json) - uoff,
                           "%s\"%s, %s\": %llu", first ? "" : ", ", disk,
                           op_name(next_key.op), uval);
        first = 0;
      }
      key = next_key;
      prev = &key;
  ...
  fprintf(f, "{\"schema_version\": 2, "
             "\"setup\": {\"generated\": %llu, \"dropped\": %llu}, "
             "\"complete\": {\"generated\": %llu, \"dropped\": %llu}, "
             "\"incomplete\": %llu, "
             "\"untracked\": %s}\n",
          totals[0], totals[1], totals[2], totals[3], incomplete, untracked_json);
  fclose(f);
  fprintf(stderr, "Counters: setup(gen=%llu) complete(gen=%llu drop=%llu) "
                  "incomplete=%llu untracked=%llu -> %s\n",
          totals[0], totals[2], totals[3], incomplete, untracked_total, path);
}
...
static int handle_event(void *ctx, void *data, size_t data_sz) {
  ...
  //
  // One row per command, so there is no `event` column and no empty-latency
  // branch: every row is a completion and always carries a real latency.
  char line[256];
  int n = snprintf(line, sizeof(line), "%llu,%llu,%s,%u,%llu,%llu,0x%llx,%u,%s,%d,%d,%s,%u\n",
                   e->timestamp_ns, e->mntns_id, op_name(e->op), e->bytes, e->latency_ns,
                   e->sector, e->rq, e->tid, comm, e->inflight_at_setup, e->inflight,
                   disk_name, e->qid);
...
int main(int argc, char **argv) {
  ...
  fprintf(output,
          "timestamp_ns,mntns_id,op,bytes,latency_ns,sector,rq,tid,comm,inflight_at_setup,inflight,disk_name,qid\n");
