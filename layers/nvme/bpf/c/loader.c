...
static void write_counters(struct standalone_bpf *skel, const char *csv_path) {
  ...
  fprintf(f, "{\"setup\": {\"generated\": %llu, \"dropped\": %llu}, "
             ...
             "\"untracked\": %s}\n",
          totals[0], totals[1], totals[2], totals[3], untracked_json);
  fclose(f);
  fprintf(stderr, "Counters: setup(gen=%llu drop=%llu) complete(gen=%llu drop=%llu) "
                  "untracked=%llu -> %s\n",
          totals[0], totals[1], totals[2], totals[3], untracked_total, path);
}
...
static int handle_event(void *ctx, void *data, size_t data_sz) {
  ...
  if (e->latency_ns > 0)
    fprintf(output, "%llu,%llu,%s,%s,%u,%llu,%llu,0x%llx,%s,%d,%s,%u\n", e->timestamp_ns,
            e->mntns_id, event_name(e->event_type), op_name(e->op), e->bytes, e->latency_ns,
            e->sector, e->rq, comm, e->inflight, disk_name, e->qid);
  else
    fprintf(output, "%llu,%llu,%s,%s,%u,,%llu,0x%llx,%s,%d,%s,%u\n", e->timestamp_ns,
            e->mntns_id, event_name(e->event_type), op_name(e->op), e->bytes, e->sector,
            e->rq, comm, e->inflight, disk_name, e->qid);
