...
const volatile char comm_filter[64] = {};
const volatile bool has_comm_filter = false;

#include "../bpf_core.h"

// ── Comm filter helper ──
// Returns true if current task's comm contains the filter substring
static __always_inline bool comm_matches(void)
{
	...
	// Substring search: check if comm_filter is contained in comm
	for (int i = 0; i < 16; i++) {
		if (comm[i] == '\0')
			break;
		bool match = true;
		for (int j = 0; j < 64 && comm_filter[j] != '\0'; j++) {
			if (i + j >= 16 || comm[i + j] != comm_filter[j]) {
				match = false;
				break;
			}
		}
