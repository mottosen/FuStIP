...
def parse_access_pattern(path):
    ...
    pattern is a per-device property, merged across all issuers). Other layers (fs) keep
    it under per-comm/per-container label entries. Merge from both so this works for every
